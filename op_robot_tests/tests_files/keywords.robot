...
Підготувати дані по критеріям життєвого циклу
  [Arguments]  ${criteria_lot}  ${tender}
  ${criteria_llc_data}=  test_criteria_llc_data  ${criteria_lot}  ${tender}
  Log  ${criteria_llc_data}
...
Дочекатись дати початку періоду уточнень
  ...
  ${next_status}=  Set variable if  '${MODE}' in ['aboveThresholdEU',  'aboveThresholdUA', 'competitiveDialogueUA', 'competitiveDialogueEU', 'simple.defense', 'closeFrameworkAgreementUA']  active.tendering  active.enquiries
