...
Можливість створити план закупівлі для closeFrameworkAgreementUA
  ${NUMBER_OF_ITEMS}=  Convert To Integer  ${NUMBER_OF_ITEMS}
  ${tender_parameters}=  Create Dictionary
  ...      mode=closeFrameworkAgreementUA
  ...      number_of_items=${NUMBER_OF_ITEMS}
  ...      tender_meat=${${TENDER_MEAT}}
  ...      item_meat=${${ITEM_MEAT}}
  ${DIALOGUE_TYPE}=  Get Variable Value  ${DIALOGUE_TYPE}
...
Можливість створити план закупівлі для closeFrameworkAgreementUA з вказанням періоду більше 5 років
  ...
  ${adapted_data}=  Адаптувати дані для оголошення тендера  ${tender_data}
  Log  ${adapted_data}
  ${raw_adap}=  from_unicode_to_json  ${adapted_data}
  ${clear_api_url}=  remove_protocol  ${API_HOST_URL}
  ${auth_base_url}=  Catenate  http://${API_LOGIN}:${API_PASS}@${clear_api_url}
  Create Session  session  ${auth_base_url}
  ${cookie_req}=  Head Request  session  /api/${API_VERSION}/plans
  ${cookie}=  Get From Dictionary  ${cookie_req.headers}  Set-Cookie
  Log  ${cookie}
  ${headers}  Create Dictionary  Content-Type=application/json  Set-Cookie=${cookie}
  ${resp}=  Post Request  session  /api/${API_VERSION}/plans  data=${raw_adap}  headers=${headers}
  Should Contain  ${resp.text}  {"endDate": ["Period startDate and endDate must be within 4 years for closeFrameworkAgreementUA."]}
...
Можливість створити план закупівлі для closeFrameworkAgreementUA да budget.period.startDate передує budget.period.startDate
  ...
  ${headers}  Create Dictionary  Content-Type=application/json  Set-Cookie=${cookie}
  ${resp}=  Post Request  session  /api/${API_VERSION}/plans  data=${raw_adap}  headers=${headers}
  Should Contain  ${resp.text}  {"startDate": ["period should begin before its end"]}
...
Можливість створити план закупівлі для belowThreshold з періодом більше 1 року
  ...
  ${headers}  Create Dictionary  Content-Type=application/json  Set-Cookie=${cookie}
  ${resp}=  Post Request  session  /api/${API_VERSION}/plans  data=${raw_adap}  headers=${headers}
  Should Contain  ${resp.text}  {"period": {"endDate": ["Period startDate and endDate must be within one year for belowThreshold."]}}